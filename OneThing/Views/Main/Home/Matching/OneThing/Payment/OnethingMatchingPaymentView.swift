    ...
    var body: some View {
        ...
        OTBaseView(String(describing: Self.self)) {
            ...
            .fullScreenCover(isPresented: $isTossPaymentSheetShown) {
                TossPaymentsView(
                    isShowingFullScreen: $isTossPaymentSheetShown,
                    onethingMatchingStore: $store
                )
            }
            .onChange(of: self.store.state.paymentResult) { _, newValue in
                self.isTossPaymentSheetShown.toggle()
                if let result = newValue, case .success = result {
                    self.homeCoordinator.push(to: .home(.onething(.paySuccess)))
                } else {
                    self.homeCoordinator.push(to: .home(.onething(.payFail)))
                }
            }
        }
        .navigationBar(
...
#Preview {
    ...
    OnethingMatchingPaymentView(store: .constant(onethingMatchingStoreForPreview))
}
