//
//  SignUpPhoneNumberView.swift
//  OneThing
//
//  Created by 윤동주 on 4/1/25.
//

import SwiftUI

struct SignUpPhoneNumberView: View {
    
    @Binding var authPathManager: OTAuthPathManager
    
    @State var text: String = ""
    @State var nicknameRule: NicknameRule = .normal
    @State var isClickable: Bool = false
    
    @Binding var viewModel: SignUpViewModel
    
    
    var body: some View {
        VStack(spacing: 0) {
            OTNavigationBar(
                leftButtonType: .back,
                title: "회원가입",
                phase: 1,
                leftAction: {
                    self.authPathManager.pop()
                },
                rightAction: {}
            )
            
            VStack(spacing: 0) {
                HStack {
                    VStack(alignment: .leading, spacing: 4) {
                        Text("원띵과 함께할 회원님의")
                            .otFont(.subtitle2)
                            .fontWeight(.semibold)
                            .foregroundStyle(.gray600)
                        Text("휴대폰 번호를 알려주세요")
                            .otFont(.heading2)
                            .fontWeight(.bold)
                            .foregroundStyle(.gray800)
                    }
                    Spacer()
                }
                .padding(.top, 32)
                
                TextField("휴대폰 번호", text: $text)
                    .keyboardType(.numberPad)
                    .padding(.horizontal, 17)
                    .frame(height: 60)
                    .background(.gray100)
                    .cornerRadius(12)
                    .padding(.top, 32)
                    .onChange(of: text) { oldValue, newValue in
                        
                        let digits = newValue.filter { $0.isNumber }
                        let limited = String(digits.prefix(11))
                        
                        text = limited.withHypen
                        
                        isClickable = (limited.count == 10 || limited.count == 11)
                        && limited.hasPrefix("01")
                    }
                
                Spacer()
            }
            .padding(.horizontal, 17)
            
            Rectangle()
                .fill(.gray200)
                .frame(height: 1)
            OTXXLButton(
                buttonTitle: "다음",
                action: {
                    Task {
                        let result = try await viewModel.updatePhoneNumber(phoneNumber: text)
                        if result {
                            authPathManager.push(path: .signUpName)
                        } else {
                            print(text)
                        }
                    }
                },
                isClickable: isClickable)
            .padding(.horizontal, 17)
        }
        .navigationBarBackButtonHidden()
    }
}

extension String {
    /// 숫자만 뽑아서 XXX-XXXX-XXXX 형태로 하이픈 삽입
    public var withHypen: String {
        // 1. 숫자만 남기기
        let digits = self.filter { $0.isNumber }
        // 2. 최대 11자리까지만
        let capped = String(digits.prefix(11))
        // 3. 자리수에 따라 하이픈 삽입
        var result = capped
        if result.count > 3 {
            result.insert("-", at: result.index(result.startIndex, offsetBy: 3))
        }
        if result.count > 8 {
            // 앞에서 삽입된 하이픈까지 포함한 인덱스 8 (원래 숫자 7번째 뒤)
            result.insert("-", at: result.index(result.startIndex, offsetBy: 8))
        }
        return result
    }
}

#Preview {
    SignUpPhoneNumberView(authPathManager: .constant(OTAuthPathManager()), viewModel: .constant(SignUpViewModel()))
}
